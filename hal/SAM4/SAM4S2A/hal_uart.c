...
void hal_UART_Init(uint8_t channel, uint32_t baud) {
    if (channel) {
		...
		hal_UART_RxChar(channel);
    hal_UART_Enable(channel);
		
		NVIC_DisableIRQ(UART0_IRQn);
		NVIC_ClearPendingIRQ(UART0_IRQn);
		NVIC_SetPriority(UART0_IRQn, 1); //Priority is 1
...
uint8_t hal_UART_TxInterruptEnabled(uint8_t channel) {
    if (channel) {
        return (UART1->UART_IMR & UART_IMR_TXEMPTY) ? 1 : 0;
    } else {
        return (UART0->UART_IMR & UART_IMR_TXEMPTY) ? 1 : 0;
    }
}
