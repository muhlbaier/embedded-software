...
//#endif
//#ifndef EEPROM_DATA2
//#define EEPROM_DATA2 0x0003F800
//#endif
...
//#define m_eeprom_start                 0x0003F000
//#define m_eeprom_size                  0x00001000
//...
...
void hal_EEPROM_Write(uint16_t address, uint16_t data) {
	...
	//if(i >= 512) {
		...
		//for(i = 0; i < EEPROM_SIZE; i++) {
			...
			//if(u32.word[0] != 0xFFFF) {
				//DisableInterrupts();
				//FlashProgram(&flashSSDConfig, write_location, 4, (uint8_t *)&u32.phrase, g_FlashLaunchCommand); // returns result code
				//EnableInterrupts();
				//write_location += 4;
			//}
		//}
	...
	//}else {
		...
		//u32.word[0] = data;
		//DisableInterrupts();
		//FlashProgram(&flashSSDConfig, (uint32_t)(data_page_ptr + i), 4, ((uint8_t *)&u32.phrase), g_FlashLaunchCommand); // returns result code
		//EnableInterrupts();
		//_next_location = i + 1;;
	//}
