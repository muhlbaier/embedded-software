...
volatile uint32_t time_ms, rollover_time, time_ms_ungated;
...
uint8_t timer_enabled = 1;  // used to gate timer
...
void Timing_EnableTimer(void) {
    timer_enabled = 1;
}

void Timing_DisableTimer(void) {
    timer_enabled = 0;
}

uint8_t Timing_IsEnabled(void) {
    return timer_enabled;
}

uint32_t TimeNowUngated(void) {
    return time_ms_ungated;
}
