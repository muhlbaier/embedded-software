...
void Bootloader_DeinitAppModules(void) {
	...
	*/
	hal_UART_Disable(SUBSYSTEM_UART);
    AlenconModbus_Stop();
}
...
void JumpToAppStateMachine(uint8_t state) 
{
	...
	switch(state) 
	{
		...
		case 1:
		{
			...
			UART_printf(SUBSYSTEM_UART, "jumping to Main App...\r\n");
...
void Bootloader_Init(void)
{
	...
	UART_printf(SUBSYSTEM_UART, "bootloader ready %d...\n\r",jump_to_app);
    while(1) 
    {
        if(jump_to_app) 
        {
        ...
        else 
        {
            ...
            if(TimeSince(status_led_time) > STATUS_LED_FLASH_PERIOD) 
            {
				...
				//debugMSG(0, "CNT: %d\n",cnt++);
				UART_printf(SUBSYSTEM_UART, "CNT: %d\n\r",cnt++);
                status_led_time = TimeNow();
                // Restart the watchdog so it doesn't reset.
                //HAL_WDOG_Refresh(WDOG);
            }
